...
from ..data.io import HDF5Manager
...
class ExperimentOrchestrator:
    ...
    _models: Dict[str, Any] = field(init=False, default_factory=dict)   # Stores registered models
    ...
    def _get_file_list(self) -> List[str]:
        # Get all .h5 files
        all_files = [f for f in os.listdir(self.data_dir) if f.lower().endswith('.h5')]

        if self.dataset == 'all':
            return sorted(all_files)

        # Normalize for comparison: lowercase and strip spaces
        search_term = str(self.dataset).strip().lower()
